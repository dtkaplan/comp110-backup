...
```{r echo=FALSE, cache=TRUE, fig.cap="Figure 1. Stock prices for Ford, Honda, and Toyota", message=FALSE, warning=FALSE, fig.margin =TRUE}
...
```{r echo=FALSE, cache=TRUE, fig.cap="Figure 2. Accumulated dividends (per share) paid by Ford, Honda, and Toyota", fig.margin = TRUE}
companies <- c("F", "TM", "HMC")
Dividends <- 
...
Figures 1 <!-- fig:auto-stock-prices--> and 2 <!-- fig:auto-stock-dividends--> show the scale of price fluctuations and of accumulated dividends. 
...
* Plot out the "closing price" (`Close`) versus `date` to get a graphic like Figure 1. <!--fig:auto-stock-prices-->
...
`r dtkbook::margin_table(Actions, show_rows = 2, caption = "The Actions table constructed by the previous command.")`
...
> *Hints*: (1) What kind of join should you use so that you get only those cases that match one of the dates in the `Actions` table? (2) Wide-vs-long techniques will be useful. (See *Data Computing* Chapter 11.<!-- chap:wide-long-->)
...
SalesDifference %>%
  mutate(profit = signif(sell - buy,3), 
         percent = signif(100*(sell - buy)/buy, 2)) %>%
  select(-buy, -sell) -> Display3
```

`r dtkbook::margin_table(Display3, show_rows = 3, caption = "Profit and percentage change in price.")`

## Indexing Prices

Since stock prices vary markedly from one company to another, a common practice is to "index" the price to a particular date as in Figure 3. <!--fig:auto-stock-index-->  (Question: In the graph, roughly which date was used for the reference?)
...
```{r echo=FALSE, fig.margin = TRUE, fig.cap="Figure 3. Indexed stock prices for Ford, Honda, and Toyota"}
...
PeriodOfInterest %>%
  ...
  group_by(company) %>%
  summarise(total_dividend=sum(dividend_per_share)) -> Interest_results
``` 

`r dtkbook::margin_table(data.frame(Interest_results), show_rows = 3, caption = "Table 1. Dividends (in dollars per share) paid out by each company over the interval 2005 through 2014.")`


* Find the total amount of dividends for each company during the period of interest.  Compare this amount to the profit (or loss) from buying and selling the stock certificates.  For the car companies, the result for the period 2005-01-01 though 2014-12-31 is shown in Table 1 <!--tab:dollar-dividends-->.
