...
Using the `Minneapolis2013` data table, answer these questions:\marginnote{\texttt{summarise()}, \texttt{group\_by()}, and \texttt{tally()} will be useful in answering the questions.}
...
Minneapolis2013 %>%
  ...
  head(3)
```
#. Which `Precinct` had the highest fraction of `First` vote selections marked as "undervote"? \marginnote[-4mm]{To calculate the fraction, use \texttt{mean(First == "undervote")} in the argument to \texttt{summarise()}.}
    ```{r echo=params$show_answers, results="hide"}
Minneapolis2013 %>%
  group_by(Precinct) %>%
