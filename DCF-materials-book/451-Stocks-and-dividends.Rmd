...
---
title: "Week 4 Activity: Joining the Stock Market"
author: "Data & Computing Fundamentals"
output:
  rmarkdown::tufte_handout:
    includes:
...
Many companies are publicly traded. \index{stocks}\index{dividends} This means that the company issues stock certificates which can be bought and sold on an exchange. Investors buy stock certificates mainly because they can sell them in the future, perhaps making a profit, and because companies pay dividends, a share of the company's profit, to each shareholder.




```{r echo=FALSE, cache=TRUE, fig.margin=TRUE, fig.cap="\\label{fig:auto-stock-prices} Stock prices for Ford, Honda, and Toyota"}
...
```{r echo=FALSE, cache=TRUE, fig.margin=TRUE, fig.cap="\\label{fig:auto-stock-dividends} Accumulated dividends (per share) paid by Ford, Honda, and Toyota"}
companies <- c("F", "TM", "HMC")
Dividends <- 
...
```{r echo=FALSE, in_margin=TRUE, title="\\texttt{Actions}", caption="\\captionof{table}{\\label{tab:stock-actions} A suggested table format for the buy and sell dates.}"}
Actions
```
...
```{r echo=FALSE, in_margin=TRUE, move_in_margin="2cm", caption="\\captionof{table}{\\label{tab:profit-calc} Profit from each of the stocks.}"}
SalesDifference %>%
...
```{r echo=FALSE, in_margin=TRUE, move_in_margin="-2cm", caption="\\captionof{table}{\\label{tab:dollar-dividends} Dividends (in dollars per share) paid out by each company over the interval 2005 through 2014.}"}
PeriodOfInterest <-
  Dividends %>%
