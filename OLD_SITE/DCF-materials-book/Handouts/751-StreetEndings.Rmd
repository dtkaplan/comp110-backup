...
table_text <- xtable::xtable(Display_sample %>% sample_n(size = 10), 
                             format = "latex")
```
...
<!-- some other styles
\marginnote{`r paste("\\noindent ", Display_sample$address[1:10],  collapse = "\\newline\n")` }

\marginnote{`r table_text`}

\marginnote{`r paste("\\noindent ", LETTERS[1:5],  collapse = "\\newline\n")`}
...
```{r echo = FALSE}
options("xtable.floating" = FALSE)
table_text <- xtable::xtable(head(LeftOvers), format = "latex")
```

\marginnote[-3cm]{`r table_text`
