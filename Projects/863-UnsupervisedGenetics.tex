...
\hline
\end{tabular}} \marginnote[0cm]{... and so on for 32 rows.}
\marginnote[0cm]{Table 1. Part of the \texttt{mtcars} data table}
...
\hline
A\\_23\\_P100001 & BR.B & -4.58\\
\hline
A\\_23\\_P100001 & BR.H & -4.57\\
\hline
A\\_23\\_P100001 & BR.MC & 1.50\\
\hline
A\\_23\\_P100001 & BR.MD & -4.30\\
\hline
A\\_23\\_P100001 & BR.T & 2.16\\
\hline
\end{tabular}} \marginnote[0cm]{... and so on for 1940640 rows.}
\marginnote[0cm]{Table 2. NCI60 gathered into a narrow form. There is a row for each probe at each of the 60 cell lines.}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Narrow <-}\StringTok{ }\NormalTok{NCI60 %>%}\StringTok{ }\KeywordTok{gather}\NormalTok{(}\DataTypeTok{value =} \NormalTok{expression, }
    \DataTypeTok{key =} \NormalTok{cellLine, -Probe) %>%}\StringTok{ }\KeywordTok{group_by}\NormalTok{(Probe, }
    \NormalTok{cellLine) %>%}\StringTok{ }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{expression =} \KeywordTok{mean}\NormalTok{(expression)) %>%}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{keep <-}\StringTok{ }\DecValTok{500}
\NormalTok{Best <-}\StringTok{ }\NormalTok{Narrow %>%}\StringTok{ }\KeywordTok{group_by}\NormalTok{(Probe) %>%}\StringTok{ }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{spread =} \KeywordTok{sd}\NormalTok{(expression)) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{arrange}\NormalTok{(}\KeywordTok{desc}\NormalTok{(spread)) %>%}\StringTok{ }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{i =} \KeywordTok{row_number}\NormalTok{()) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{head}\NormalTok{(keep)}
\NormalTok{Randomized <-}\StringTok{ }\NormalTok{Narrow %>%}\StringTok{ }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Probe =} \KeywordTok{shuffle}\NormalTok{(Probe)) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{group_by}\NormalTok{(Probe) %>%}\StringTok{ }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{spread =} \KeywordTok{sd}\NormalTok{(expression)) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{arrange}\NormalTok{(}\KeywordTok{desc}\NormalTok{(spread)) %>%}\StringTok{ }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{i =} \KeywordTok{row_number}\NormalTok{()) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{head}\NormalTok{(keep)}
\NormalTok{Best %>%}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{i, }\DataTypeTok{y =} \NormalTok{spread)) +}\StringTok{ }\KeywordTok{geom_line}\NormalTok{() +}\StringTok{ }
\StringTok{    }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{data =} \NormalTok{Randomized, }\DataTypeTok{color =} \StringTok{"red"}\NormalTok{, }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Keepers <-}\StringTok{ }\NormalTok{Narrow %>%}\StringTok{ }\KeywordTok{group_by}\NormalTok{(Probe) %>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(}\KeywordTok{sd}\NormalTok{(expression) >}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Keepers <-}\StringTok{ }\NormalTok{Keepers %>%}\StringTok{ }\KeywordTok{spread}\NormalTok{(}\DataTypeTok{key =} \NormalTok{Probe, }\DataTypeTok{value =} \NormalTok{expression)}
\NormalTok{scale_colour_discrete <-}\StringTok{ }\NormalTok{function(...) }\KeywordTok{scale_colour_hue}\NormalTok{(...)}
\end{Highlighting}
\end{Shaded}
